...
В этом сегменте половина вопросов задаётся скриншотом: человек кидает картинку
и спрашивает «а это что». Поэтому вложение едет в конверт отдельной строкой —
`[изображение: <что на нём>]`, `[голосовое: <расшифровка>]` — сразу после
маркера поста или перед подписью автора. Нечитаемое вложение всё равно даёт
строку (`[изображение: описание недоступно]`): модель должна знать, что
картинка была, и не выдумывать её содержимое. Правило одно на оба рантайма
(`transcript.media_context`, `MediaContext` в `group.go`) и покрыто голденом.

Описание картинок и расшифровка голосовых считаются на ответном пути:
vision-алиас ai-gateway и whisper-предиктор. На пути наблюдения они не
запускаются — описывать каждый пролетающий скриншот значит платить за vision
на сообщениях, которых никто не спрашивал; в корпус едет вид вложения, и
`chat_search` ищет по нему вместе с текстом.
...
- комментарий к посту-картинке приезжает без самой картинки: в конверте есть
  текст поста (`[в ответ на пост канала: ...]`), но не его вложение. Спросили
  «а что там на скрине» — агент видит вопрос и не видит скрин.
