...
Status: Console side IMPLEMENTED (ingest handler + route + migration 053, all verified — see §11). Hub side still to build against this contract.
Author: platform
...
**Still needs the hub repo (cannot be checked from `dada-cloud`) — this is the remaining work:**

1. Confirm the **pinned** Claude Code version in the hub image emits the same shape as 2.1.215; adjust the parser fallbacks (`total_cost_usd || cost_usd`, `modelUsage` absent) accordingly.
2. Whether the hub already parses `claude -p` stdout / already runs `stream-json` (reuse vs add the parse).
3. Confirm the hub holds `cloud_task_id` in-process for both the intent and runs/autofix paths at the point `claude -p` is spawned. Strong evidence it does `[code]`: the console reads `cloud_task_id` back from the hub via `GET /v1/runs/{id}` (`GetRun`) precisely because the hub minted and owns it — confirm hub-side.
4. Confirm the hub's outbound egress to the console callback URL is allowed (network/proxy), same path the status webhook already uses.
