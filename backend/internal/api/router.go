...
func SetupRouterWithHandler(pool *pgxpool.Pool, cfg *config.Config) (*gin.Engine, *Handler) {
	...
	if cfg.InternalAuthToken != "" {
		...
		internal.POST("/ai/credential/candidates", h.AIGetCredentialCandidates)
		internal.POST("/ai/credential/models", h.AIReportCredentialModels)
		internal.POST("/ai/key/models", h.AIGetKeyModels)
	...
	api := r.Group("/api/v1", authMW, h.auditSessionMiddleware(), clientClaimMiddleware())
	{
		// Auth
		...
		api.GET("/projects/:projectId/environments/:envId/databases/:name/tables/:table", h.GetDatabaseTable)
		api.GET("/projects/:projectId/environments/:envId/databases/:name/tables/:table/archive-plan", h.GetDatabaseArchivePlan)
		api.POST("/projects/:projectId/environments/:envId/databases/:name/archive-runs", h.StartDatabaseArchive)
		api.GET("/projects/:projectId/environments/:envId/databases/:name/archive-runs", h.ListDatabaseArchiveRuns)
		...
		api.GET("/projects/:projectId/environments/:envId/databases/:name/activity", h.GetDatabaseActivity)
		api.POST("/projects/:projectId/environments/:envId/databases/:name/activity/:pid/cancel", h.CancelDatabaseBackend)
		api.POST("/projects/:projectId/environments/:envId/databases/:name/query", h.QueryDatabase)
		...
		api.PATCH("/projects/:projectId/environments/:envId/apps/:appName/start-command", h.UpdateAppStartCommand)
		api.PATCH("/projects/:projectId/environments/:envId/apps/:appName/port", h.UpdateAppPort)
		api.PATCH("/projects/:projectId/environments/:envId/apps/:appName/upload-port", h.SetUploadPort)
		api.POST("/projects/:projectId/environments/:envId/apps/:appName/adopt-config", h.AdoptAppConfig)
		...
		api.GET("/projects/:projectId/ai/keys/:keyId/models", h.ListAIKeyModels)
		api.GET("/admin/ai-gateway/credentials", h.ListAdminAICredentials)
		api.POST("/admin/ai-gateway/credentials", h.CreateAdminAICredentials)
		api.PATCH("/admin/ai-gateway/credentials/:credentialId", h.UpdateAdminAICredential)
		api.DELETE("/admin/ai-gateway/credentials/:credentialId", h.DeleteAdminAICredential)
		api.GET("/admin/ai-gateway/models/stats", h.ListAdminAIModelStats)
