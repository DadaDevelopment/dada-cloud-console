...
// TestSlowQueryAdvisories_SilentOnAnIdleDatabase pins what the first live run
// on shard-1 exposed: share alone is not evidence. Four all-but-empty
// databases each reported a "slow query" holding 97% of their time, and the
// control plane's own database reported one with mean=0ms. Nothing there is
// worth a person's morning, so the rule has to stay quiet.
func TestSlowQueryAdvisories_SilentOnAnIdleDatabase(t *testing.T) {
	in := dbAdvisoryInput{
		Statements: []dbAdvisoryStatement{{
			QueryID:      6737800885445559477,
			Sample:       "SELECT pg_database_size($1)",
			DeltaCalls:   6,
			DeltaTotalMs: 600,
			MeanMs:       100,
		}},
		StatementsTotalMs: 620,
	}

	if got := slowQueryAdvisories(in); len(got) != 0 {
		t.Errorf("slow_query fired on a database that did 0.6s of work all window: %v", got)
	}

	in = dbAdvisoryInput{
		Statements: []dbAdvisoryStatement{{
			QueryID:      4178026152376209659,
			Sample:       "SELECT $1",
			DeltaCalls:   10_530,
			DeltaTotalMs: 5_265,
			MeanMs:       0.5,
		}},
		StatementsTotalMs: 19_600,
	}

	if got := slowQueryAdvisories(in); len(got) != 0 {
		t.Errorf("slow_query fired on a half-millisecond query: %v", got)
	}
}

// TestSlowQueryAdvisories_StillFiresOnABusyDatabase is the other half: the
// floors must not silence a query that genuinely holds a working database.
func TestSlowQueryAdvisories_StillFiresOnABusyDatabase(t *testing.T) {
	in := dbAdvisoryInput{
		Statements: []dbAdvisoryStatement{{
			QueryID:      42,
			Sample:       "SELECT ... FROM events WHERE kind = $1",
			DeltaCalls:   12_400,
			DeltaTotalMs: 21_000,
			MeanMs:       1.7,
		}},
		StatementsTotalMs: 66_100,
	}

	got := slowQueryAdvisories(in)
	if len(got) != 1 {
		t.Fatalf("slow_query silent on a query holding 32%% of a busy database: %v", got)
	}
	if got[0].Severity != dbAdvisoryWarning {
		t.Errorf("severity = %q, want warning for dominant-but-fast", got[0].Severity)
	}
}
