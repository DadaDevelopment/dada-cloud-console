...
func (h *Handler) AttachHostname(c *gin.Context) {
	...
	// A domain is an HTTP promise. Do not create a hostname that has no
	// configured public port; port zero is the explicit no-route state.
	var appPort int
	if err := h.pool.QueryRow(c.Request.Context(),
		`SELECT COALESCE((summary_json->>'port')::int, 0)
		 FROM resource_snapshots
		 WHERE project_id = $1 AND environment_id = $2 AND kind = 'App' AND name = $3
		 LIMIT 1`,
		projectID, envID, appName,
	).Scan(&appPort); err == pgx.ErrNoRows {
	...
	if appPort <= 0 {
		rejectAttach(http.StatusConflict, "app_has_no_public_port", "configure a public port before attaching a hostname")
		return
	}
