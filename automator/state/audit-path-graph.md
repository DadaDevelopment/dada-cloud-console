...
# 2026-09-03 sess-0903c — путь новых юзеров окна 08-25..09-03 (live psql)

Источник: psql cloud-console (audit_events/ux_events/payments), окно с 2026-08-25.
Новых реальных юзеров: 4 за 7д (zqleaders 08-25, m206rv159 08-26, kof97zip 08-28,
messiajit4 08-28) + saravananofficial13 09-02 + wgck 09-03. users total = 62.

## Цепочки (читаю как историю)

- zqleaders (08-25): SignUp -> project -> 2x CreateAppServer -> GitHub-install ->
  ConnectGitRepo dada-argogo -> build success 80с -> CreateApp -> ViewApp.
  ТЕРМИНАЛ: ViewApps 03:36. Активирован полностью за 38 мин, потом ушёл.
  Форма: «собрал и ушёл» (5-й случай класса).
- m206rv159 (08-26): OAuth-стена держала его 30 мин (2x StartGitAppInstall без
  Finish, 11:16 вернулся) -> bookstore build success -> CreateServiceDatabase
  СЕМЬ попыток за 4 мин (имена bookstore_db, BookstoreDb, "Db bookstore"...)
  -> RevealDatabaseCredentials x4 -> SeedDatabaseDSN. ТЕРМИНАЛ: CreateApp 08-30.
  Гипотеза UX: создание БД требует нескольких попыток, чтобы догадаться про
  имя/тип; юзер делает это методом проб. Инструмент: у CreateServiceDatabase
  нет outcome=failure-причины в audit (успех-строки только) - фрикция
  невидима, только косвенно по серии переименований.
- kof97zip (08-28): SignUp -> 2x CreateApp myapp -> терминал ViewApps 03:00.
  Создал апп руками, НИКАКОГО билда/репо. Похоже на тыканье в UI без проекта.
- messiajit4 (28-08, соц-SSO): ЛЕГЕНДАРНЫЙ путь - SignUp -> install -> build ->
  DeployImageVersion x2 -> UpdateAppStartCommand -> SetEnvVar -> 2 БД ->
  SeedDatabaseDSN за 1ч - потом тишина 4 дня (уже знал из 09-01).
  Возврата нет. Сильнейший юзер ушёл сразу после успеха.
- saravananofficial13 (09-02): его app it-tools-8ry61n - единственный failed
  build 7д: pnpm-lock.yaml не содержит @pnpm/exe.linux-x64 (npm i -g pnpm в
  Dockerfile). Класс «юзерский Dockerfile против frozen lockfile» - это юзерский
  код, не наша инфра. Но есть неудобство: платформа говорит только
  «dockerfile_build_failed», приговор можно понять, но текст ошибки на 3 экрана.
- wgck (09-03, СЕГОДНЯ): SignUp 01:36 -> UploadSourceArchive (ПОТОК 1!) ->
  build success за ~65с -> CreateApp 01:40. Первым действием выбрал ЗАГРУЗКУ
  АРХИВА, не git. 4 минуты до аппа. Поток 1 работает на живом незнакомце.

## Переходы (свертка за окно)

Горячие: BuildFinished->DeployImageVersion 149/6юз, DeployImageVersion->x2 96/7,
ViewProject->ViewApps 95/12, SessionStart->ViewProject 68/6,
UploadSourceArchive->ViewBuildLogs 15/3. Терминалы новых юзеров: 3x ViewApps,
1x CreateApp - т.е. уходят СРАЗУ после успеха/пустого списка, не на ошибке.

## UX-выводы (обязательный пункт)

1. «Собрал и ушёл» - доминирующий финал. Ни один новый юзер не вернулся
   вторым днем. Продукт выигрывает первую сессию и не даёт причины вернуться.
   Кандидат-рычаг остаётся в backlog (письма запрещены, канал = продукт:
   уведомление в консоли/ТГ-бот от аппа, health-дайджест).
2. Фрикция создания БД невидима: 7 попыток m206 не оставили failure-строк.
   Инструментирование: в CreateServiceDatabase нет отказов как класса, потому
   что валидация на клиенте молча мешает. Нужно ux-событие на отказ валидации
   формы БД (файл/строку зафиковали в backlog, не в этом цикле).
3. 4 feedback-строки artempro2021 (08-24/25) разобраны ПОИМЕННО:
   - порт-валидация и слетающий порт: УЖЕ починено владельцем 3613dc49 08-25
     (описание совпадает 1:1: форма читала любой 400 как range-error);
   - «Лимиты Free не применяются» - это РАБОТАЕТ КАК ЗАДУМАНО (грейс до 25.09,
     owner-информирован через P1-PAY-7);
   - fanclub.run.place verify: разобран live в этом цикле (см ниже) - домен
     РАБОТАЕТ с 08-25 12:25 (проверка DNS+TLS+200 из 6 стран). Юзер написал
     фидбек в 09:33, починилось владельцем в 12:25 (73aa9504+4dfa6319). РЕАКЦИЯ
     ЦИКЛА: сообщение юзеру не нужно (письма запрещены), но фронт-индикатор
     этого домена в консоли должен показывать active - сверил: hostname row
     active/active. Действий не требуется.
   - «DNS указывает на ingress, но Ingress-правила нет»: тот же домен,
     написано ДО верификации (11:56 vs verify 12:25). Сейчас правило есть.
     ТИХИЙ РЕЗЮМ: все 4 строки либо уже починены, либо ложная тревога -
     фидбек-канал работает, скорость реакции владельца < 1 дня.

## Домен fanclub.run.place - вердикт (платформа НЕ виновата, сейчас жив)

- Ingress fanclub-run-place существует, nginx-conf в pub-controller содержит
  server fanclub.run.place (listen 80/443 ssl), cert-manager CN=fanclub.run.place
  (TLS True, валиден до 23.11). curl --resolve -> 200 (6/6 попыток),
  probe-external 5/6 стран 200, http->308 redirect. DNS A = 155.212.223.198
  (8.8.8.8/1.1.1.1/77.88.8.8).
- Если у юзера не открывается: проблема на его стороне (локальный DNS-кеш
  до 08-25, run.place - бесплатный домен, резолверы провайдеров кэшируют
  NXDOMAIN эпохи до делегирования).
