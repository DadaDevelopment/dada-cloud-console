...
const ru: Dict = {
  ...
  mcpAlt: {
    ...
    heroSubtitle:
      "Подключите платформу к Claude Code, Claude Desktop, Cursor или любому MCP-клиенту — и просите словами: подними сервер, разверни репозиторий, покажи логи упавшего приложения. Агент получает 61 инструмент консоли, входит под вашим аккаунтом DADA ID через браузер и работает ровно с теми правами, что есть у вас. Адрес сервера — https://console.dada-tuda.ru/mcp.",
    ...
    features: [
      { title: "61 инструмент платформы", desc: "Проекты, приложения, сборки, базы, домены, переменные окружения, серверы, логи и одноразовые песочницы-боксы — тот же API, на котором работает веб-консоль, отобранный до набора, в котором агент не теряется." },
...
const en: Dict = {
  ...
  mcpAlt: {
    ...
    features: [
      { title: "61 platform tools", desc: "Projects, apps, builds, databases, domains, environment variables, servers, logs and throwaway sandbox boxes — the same API the web console runs on, cut down to a set an agent does not get lost in." },
