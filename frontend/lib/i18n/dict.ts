...
const ru: Dict = {
  ...
  telegramBotAlt: {
    ...
    features: [
      ...
      { title: "Работает через webhook", desc: "Бот принимает сообщения от Telegram по вебхуку: пусть приложение слушает 0.0.0.0 и порт из переменной окружения PORT, которую даёт Dada Cloud, — и бот сразу доступен снаружи. Long polling пока не поддерживается." },
    ...
    faq: [
      ...
      { q: "Как бот принимает сообщения — через webhook или long polling?", a: "Сейчас поддерживается только webhook. Приложение должно слушать 0.0.0.0 и порт из переменной окружения PORT, которую передаёт Dada Cloud, — тогда Telegram сможет достучаться до бота снаружи." },
...
const en: Dict = {
  ...
  telegramBotAlt: {
    ...
    features: [
      ...
      { title: "Runs over webhook", desc: "The bot receives Telegram updates over a webhook: have your app listen on 0.0.0.0 and the port from the PORT environment variable that Dada Cloud sets, and the bot is reachable right away. Long polling isn't supported yet." },
    ...
    faq: [
      ...
      { q: "Does the bot use webhook or long polling?", a: "Only webhook is supported right now. Your app needs to listen on 0.0.0.0 and the port from the PORT environment variable Dada Cloud sets — that's how Telegram reaches your bot from outside." },
