...
const ru: Dict = {
  ...
  renderAlt: {
    ...
    mapping: [
      { from: "Web Service", to: "Приложение из репозитория", note: "Build и start command переносятся один в один" },
      { from: "Background Worker", to: "Отдельное приложение", note: "Тот же репозиторий, своя команда запуска" },
  ...
  flyIoAlt: {
    ...
    notPortable: [
      ...
      { title: "fly.toml не читается", desc: "Конфигурация приложения живёт в панели и в API, а не в файле репозитория. Переменные, команда запуска и ресурсы задаются один раз при создании приложения." },
