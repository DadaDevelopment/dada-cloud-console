...
const ru: Dict = {
  ...
  home: {
    ...
    mcp: {
      ...
      bullets: [
        "45 инструмента платформы прямо в чате",
  ...
  mcpAlt: {
    ...
    heroSubtitle:
      "Подключите платформу к Claude Code, Claude Desktop, Cursor или любому MCP-клиенту — и просите словами: подними сервер, разверни репозиторий, покажи логи упавшего приложения. Агент получает 45 инструмента консоли, входит под вашим аккаунтом DADA ID через браузер и работает ровно с теми правами, что есть у вас. Адрес сервера — https://console.dada-tuda.ru/mcp.",
    ...
    features: [
      { title: "45 инструмента платформы", desc: "Проекты, приложения, сборки, базы, домены, переменные окружения, серверы, логи и одноразовые песочницы-боксы — тот же API, на котором работает веб-консоль, отобранный до набора, в котором агент не теряется." },
...
const en: Dict = {
  ...
  home: {
    ...
    mcp: {
      ...
      bullets: [
        "45 platform tools right in the chat",
  ...
  mcpAlt: {
    ...
    features: [
      { title: "45 platform tools", desc: "Projects, apps, builds, databases, domains, environment variables, servers, logs and throwaway sandbox boxes — the same API the web console runs on, cut down to a set an agent does not get lost in." },
