...
export const apps: Messages = {
  ...
  "apps.deploy.fromUpload.noPort.title": { ru: "Веб-порт не нашли - адрес выдать не можем", en: "No web port found - we can't issue an address" },
  "apps.deploy.fromUpload.noPort.desc": { ru: "Автоматическое определение не нашло веб-фреймворк и открытый порт в этом архиве. Если это сайт или API - укажите порт, на котором слушает сервер, и мы выдадим адрес. Если это бот или фоновый воркер без входящих запросов - адрес не нужен, продолжайте.", en: "Autodetection found no web framework and no exposed port in this archive. If this is a site or API, tell us the port your server listens on and we'll issue an address. If it's a bot or background worker with no inbound requests, no address is needed - continue." },
  "apps.deploy.fromUpload.noPort.isSite": { ru: "Это сайт или API", en: "This is a site or API" },
  "apps.deploy.fromUpload.noPort.isWorker": { ru: "Это бот / воркер, адрес не нужен", en: "This is a bot / worker, no address needed" },
  "apps.deploy.fromUpload.noPort.portLabel": { ru: "Порт, на котором слушает сервер", en: "Port your server listens on" },
  "apps.deploy.fromUpload.noPort.confirmPort": { ru: "Подтвердить порт и продолжить", en: "Confirm port and continue" },
  "apps.deploy.fromUpload.noPort.confirmWorker": { ru: "Продолжить без адреса", en: "Continue without an address" },
  "apps.deploy.fromUpload.noPort.saving": { ru: "Сохранение...", en: "Saving..." },
  "apps.deploy.fromUpload.noPort.error.invalid": { ru: "Порт должен быть в диапазоне 1..65535", en: "Port must be in the range 1..65535" },
