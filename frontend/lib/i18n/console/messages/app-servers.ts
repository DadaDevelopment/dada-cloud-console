...
export const appServers: Messages = {
  ...
  "appServers.tab.overview": { ru: "Обзор", en: "Overview" },
  "appServers.tab.workloads": { ru: "Нагрузки", en: "Workloads" },
  "appServers.tab.metrics": { ru: "Метрики", en: "Metrics" },
  "appServers.tab.logs": { ru: "Логи", en: "Logs" },

  "appServers.adopt.title": { ru: "Есть работающие контейнеры на этой VM?", en: "Already running containers on this VM?" },
  "appServers.adopt.desc": { ru: "Откройте вкладку «Нагрузки», чтобы обнаружить их и импортировать в управляемое приложение Dada — без пересборки.", en: "Open the Workloads tab to discover them and import into a managed Dada app — no rebuild." },

  "appServers.workloads.title": { ru: "Обнаружение и импорт", en: "Discover & import" },
  "appServers.workloads.subtitle": { ru: "Только чтение. Ничего не меняется, пока вы не нажмёте «Импорт».", en: "Read-only. Nothing changes until you hit Import." },
  "appServers.workloads.rescan": { ru: "Пересканировать", en: "Re-scan" },
  "appServers.workloads.importCta": { ru: "Импортировать в Dada", en: "Import into Dada" },
  "appServers.workloads.found": { ru: "Найдено контейнеров: {n}", en: "{n} containers found" },
  "appServers.workloads.none": { ru: "Работающих контейнеров не найдено.", en: "No running containers found." },
  "appServers.workloads.noPortsVols": { ru: "нет портов и томов", en: "no ports or volumes" },
  "appServers.workloads.warnings": { ru: "Предупреждения discovery", en: "Discovery warnings" },
  "appServers.workloads.scanning": { ru: "Сканируем VM через Portainer…", en: "Scanning the VM via Portainer…" },
  "appServers.workloads.empty.title": { ru: "Ещё не сканировали", en: "Nothing discovered yet" },
  "appServers.workloads.empty.desc": { ru: "Запустите discovery, чтобы увидеть контейнеры, образы, порты и тома на этой VM. Только чтение — через Portainer, без SSH.", en: "Run discovery to inventory the containers, images, ports and volumes on this VM. Read-only — via Portainer, no SSH." },
  ...
  "appServers.import.services.count": { ru: "{n} из {total} выбрано", en: "{n} of {total} selected" },
  "appServers.import.services.include": { ru: "Включить {name}", en: "Include {name}" },
  "appServers.import.services.dataSafe": { ru: "Именованный том подключается как external — данные сохраняются.", en: "Named volume attached as external — data is preserved." },
  "appServers.import.env.toggle": { ru: "Переменные окружения (.env, опционально)", en: "Environment variables (.env, optional)" },
  "appServers.import.env.consent": { ru: "Я понимаю: эти значения будут закоммичены в git в открытом виде. Не вставляйте секреты, которые нельзя хранить в репозитории.", en: "I understand these values will be committed to git in plaintext. Do not paste secrets that must not live in the repo." },
  "appServers.import.preview.toggle": { ru: "Предпросмотр compose.yaml", en: "Preview compose.yaml" },
  "appServers.import.submit": { ru: "Импортировать и развернуть", en: "Import & deploy" },
  "appServers.import.failed": { ru: "Не удалось импортировать нагрузку", en: "Failed to import workload" },
  "appServers.import.timeout": { ru: "Импорт не завершился вовремя", en: "Import did not finish in time" },
  "appServers.import.progress.queued": { ru: "Операция создана…", en: "Operation queued…" },
  "appServers.import.progress.running": { ru: "Импорт… ({status})", en: "Importing… ({status})" },
};
