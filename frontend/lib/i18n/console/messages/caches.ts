...
export const caches: Messages = {
  ...
  "caches.modal.appRef.required": { ru: "Пользователь Redis обязательно привязывается к приложению", en: "A Redis user must be bound to an application" },
  "caches.modal.keyPrefix.label": { ru: "Префикс ключей", en: "Key prefix" },
  "caches.modal.keyPrefix.hint": { ru: "Пользователь получит доступ только к ключам с этим префиксом", en: "The user is scoped to keys under this prefix only" },
  ...
  "caches.modal.submit": { ru: "Создать пользователя", en: "Create user" },
