...
export function AccountMenu() {
  ...
  const { role, projectId } = useProjectContext();
      ...
      {open && (
        <div role="menu" className="absolute right-0 z-50 mt-2 w-64 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-2xl dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="py-1">
            ...
            {projectId && isAdmin(role) && (
              <>
                <Link role="menuitem" href={`/projects/${projectId}/members`} onClick={() => setOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-800">
                  {t("nav.members")}
                </Link>
                <Link role="menuitem" href={`/projects/${projectId}/billing`} onClick={() => setOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-800">
                  {t("nav.billing")}
                </Link>
              </>
            )}
