  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center gap-2 border-b border-gray-100 px-4 py-3">
        ...
        />
        <span className="shrink-0 text-xs text-gray-400">{names.length} metrics</span>
      </div>
      <div className="max-h-96 divide-y divide-gray-50 overflow-y-auto">
        {names.length === 0 ? (
          <p className="px-4 py-8 text-center text-sm text-gray-400">No metrics match.</p>
        ) : (
          names.map((name, i) => {
            ...
            return (
              <div key={name} className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50">
                <div className="min-w-0 flex-1">
                  <p className="truncate font-mono text-xs text-gray-800">{name}</p>
                ...
                <button
                  ...
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:border-blue-300 hover:text-blue-600"
                >
