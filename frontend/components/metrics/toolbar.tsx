    ...
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 shadow-sm">
      ...
      {state.range === "custom" && (
        <Popover>
          <PopoverTrigger asChild>
            <button className="h-8 rounded-lg border border-gray-200 bg-white px-2.5 text-xs font-medium text-gray-600 shadow-sm hover:border-gray-300">
              {state.from && state.to
      ...
      {labelKeys.length > 0 && (
        <Popover>
          ...
          <PopoverContent className="w-72 space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Label filters</p>
            {labelKeys.map((k) => (
              <label key={k} className="block text-xs font-medium text-gray-500">
                {k}
                <select
                  ...
                  className="mt-1 block w-full rounded-lg border border-gray-300 px-2 py-1 text-sm text-gray-900"
                >
      ...
      <div className="ml-auto flex items-center gap-2">
        <button
          onClick={onRefresh}
          title="Refresh now"
          className="flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 shadow-sm hover:border-gray-300 hover:text-gray-700"
        >
