...
/**
 * Shape of every "alt" landing entry in the marketing dictionary. The optional
 * blocks are what separates a page that ranks from a thin one, so they render
 * only when the dictionary actually fills them in.
 */
export type AltLanding = {
  heroTitle: string;
  heroSubtitle: string;
...
/** Pricing table plus amber caveat cards. Both blocks are optional in the dictionary. */
export type PricingAndCaveatsProps = {
  mappingTitle?: string;
  mappingSubtitle?: string;
  mapping?: MappingRow[];
  notPortableTitle?: string;
...
/**
 * Tier table and "worth knowing up front" cards, shared by the alt landings and
 * by pages that keep their own hero and step markup.
 */
export function PricingAndCaveats(g: PricingAndCaveatsProps) {
  const { t } = useLang();
  if (!g.mapping || g.mapping.length === 0) return null;
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead className="border-b border-slate-200 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
              <tr>
                <th className="px-6 py-3">{t.common.tierColumn}</th>
                <th className="px-6 py-3">{t.common.priceColumn}</th>
                <th className="px-6 py-3">{t.common.noteColumn}</th>
              </tr>
            </thead>
...
/**
 * Renders a complete competitor/use-case landing from one dictionary entry:
 * hero, features, how-to steps, a pricing table, caveats and FAQ, plus the
...
export function AltLandingPage({ path, g, utm }: { path: string; g: AltLanding; utm: string }) {
  ...
  return (
    <>
      ...
      <ProductHero title={g.heroTitle} subtitle={g.heroSubtitle} ctaHref={ctaHref} />
