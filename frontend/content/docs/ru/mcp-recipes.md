...
listBoxAttachments(projectId, boxName="scratch")   → какие КЛЮЧИ окружения проброшены
exposeBox(projectId, boxName="scratch", port=3000) → выданное платформой имя хоста
```

База создаётся *снаружи* box и пробрасывается в его env-файл, поэтому удаление box никогда не
