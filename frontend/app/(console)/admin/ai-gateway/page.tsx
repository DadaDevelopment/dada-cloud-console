        ...
        {isLoading ? <p className="py-5 text-center text-sm text-gray-400">…</p> : credentials.length === 0 ? <p className="py-5 text-center text-sm text-gray-400">{t("aiGateway.pool.empty")}</p> : credentials.map((credential) => (
          <div key={credential.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-gray-200 dark:border-gray-800 px-4 py-3">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2"><span className="font-medium text-gray-900 dark:text-gray-100">{credential.label || credential.provider}</span><code className="text-xs text-gray-400">{credential.key_hint}</code>{credential.enabled ? <span className="rounded-full bg-emerald-100 dark:bg-emerald-950/50 px-2 py-0.5 text-xs text-emerald-700 dark:text-emerald-300">{t("aiGateway.pool.enabled")}</span> : <span className="text-xs text-gray-400">{t("aiGateway.pool.disabled")}</span>}</div>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{credential.provider}{credential.api_base ? ` · ${credential.api_base}` : ""} · {t("aiGateway.pool.priority", { priority: credential.priority })}</p>
            </div>
            <div className="flex flex-wrap gap-2"><button type="button" onClick={() => beginEdit(credential)} className="rounded-lg border border-gray-300 dark:border-gray-700 px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300">{t("aiGateway.pool.edit")}</button><button type="button" onClick={() => void toggleCredential(credential)} className="rounded-lg border border-gray-300 dark:border-gray-700 px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300">{credential.enabled ? t("aiGateway.pool.disable") : t("aiGateway.pool.enable")}</button><button type="button" onClick={() => void deleteCredential(credential)} className="rounded-lg border border-red-200 dark:border-red-900 px-3 py-1.5 text-sm text-red-600 dark:text-red-400">{t("aiGateway.pool.delete")}</button></div>
