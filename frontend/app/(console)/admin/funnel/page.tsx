...
import { FunnelSpine } from "@/components/console/funnel-spine";
...
export default function AdminFunnelPage() {
  ...
  /**
   * Resolves a breakdown part to a human label. Signup doors and resource
   * kinds already have translations; anything unknown keeps its raw key rather
   * than being hidden, so a new door shows up the day it appears.
   */
  const sliceLabel = (stageKey: string, partKey: string): string => {
    if (stageKey === "requested") return t(`adminFunnel.resource.${partKey}`);
    const known = `adminFunnel.journey.door.${partKey}`;
    const translated = t(known);
    return translated === known ? partKey : translated;
  };
  ...
  return <div>
    ...
    <div className="my-4 flex flex-wrap items-center gap-2"><div className="flex shrink-0 items-center gap-1 rounded-lg bg-gray-100 p-0.5 dark:bg-gray-800">{WINDOWS.map((window) => <button key={window} type="button" onClick={() => setWindowKey(window)} className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${windowKey === window ? "bg-white text-gray-900 shadow-sm dark:bg-gray-700 dark:text-gray-100" : "text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"}`}>{t(`adminFunnel.window.${window}`)}</button>)}</div><div className="flex shrink-0 items-center gap-1 rounded-lg bg-gray-100 p-0.5 dark:bg-gray-800"><span className="px-1.5 text-xs text-gray-500 dark:text-gray-400">{t("adminFunnel.journey.scaleLabel")}</span>{(["log", "linear"] as const).map((mode) => <button key={mode} type="button" onClick={() => setScale(mode)} className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${scale === mode ? "bg-white text-gray-900 shadow-sm dark:bg-gray-700 dark:text-gray-100" : "text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"}`}>{t(mode === "log" ? "adminFunnel.journey.scaleLog" : "adminFunnel.journey.scaleLinear")}</button>)}</div></div>
    <section className="mb-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900"><h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">{t("adminFunnel.journey.title")}</h2><p className="mt-1 max-w-4xl text-sm text-gray-500 dark:text-gray-400">{t("adminFunnel.journey.body")}</p>{loading ? <div className="flex h-96 items-center justify-center"><Spinner size="md" /></div> : journey && journey.stages.length > 0 && journey.stages[0].value > 0 ? <div className="mt-5"><FunnelSpine stages={journey.stages} slices={journey.slices} darkSegment={journey.dark_segment} darkNote={t("adminFunnel.journey.darkNote")} scale={scale} sliceLabel={sliceLabel} ariaLabel={t("adminFunnel.journey.title")} /></div> : <p className="mt-4 text-sm text-amber-600 dark:text-amber-400">{t("adminFunnel.journey.empty")}</p>}</section>
    <section className="mb-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900"><h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">{t("adminFunnel.crossCheck.title")}</h2><p className="mt-1 max-w-4xl text-sm text-gray-500 dark:text-gray-400">{t("adminFunnel.crossCheck.body")}</p>{loading ? <div className="flex h-72 items-center justify-center"><Spinner size="md" /></div> : <><div className="mt-5"><DetailedFunnelSankey streams={acquisitionStreams} annotations={[{ label: t("adminFunnel.channel.visits"), value: traffic?.totals.visits ?? 0, detail: t("adminFunnel.acquisition.visitsDetail") }, { label: t("adminFunnel.channel.deploy"), value: traffic?.totals.deploy_success ?? 0, detail: t("adminFunnel.acquisition.deployDetail") }]} ariaLabel={t("adminFunnel.crossCheck.title")} /></div>{!traffic?.available && <p className="mt-4 text-sm text-amber-600 dark:text-amber-400">{traffic?.note || t("adminFunnel.channel.unavailable")}</p>}<details className="mt-5 border-t border-gray-100 pt-4 dark:border-gray-800"><summary className="cursor-pointer text-xs font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">{t("adminFunnel.channel.table")}</summary><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead className="border-b border-gray-200 text-left text-xs text-gray-500 dark:border-gray-800 dark:text-gray-400"><tr><th className="px-2 py-2">{t("adminFunnel.channel.source")}</th><th className="px-2 py-2 text-right">{t("adminFunnel.channel.visits")}</th><th className="px-2 py-2 text-right">{t("adminFunnel.channel.register")}</th><th className="px-2 py-2 text-right">{t("adminFunnel.channel.started")}</th><th className="px-2 py-2 text-right">{t("adminFunnel.channel.complete")}</th></tr></thead><tbody>{traffic && [...traffic.channels, traffic.totals].map((row, index) => <tr key={`${row.source}-${index}`} className="border-t border-gray-100 text-gray-700 dark:border-gray-800 dark:text-gray-300"><td className="px-2 py-2">{index === traffic.channels.length ? t("adminFunnel.channel.total") : trafficSourceLabel(row.source)}</td><td className="px-2 py-2 text-right tabular-nums">{row.visits}</td><td className="px-2 py-2 text-right tabular-nums">{row.register_opened}</td><td className="px-2 py-2 text-right tabular-nums">{row.signup_started}</td><td className="px-2 py-2 text-right tabular-nums">{row.registration_complete}</td></tr>)}</tbody></table></div></details></>}</section>
