    ...
    <div>
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          />
          <h1 className="mt-2 text-2xl font-bold text-gray-900 dark:text-gray-100">{t("git.title")}</h1>
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">{t("git.subtitle")}</p>
        </div>
        {canConnect && (
          <Link
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 py-16">
          ...
          <p className="mt-3 max-w-sm text-center text-xs text-gray-400 dark:text-gray-500">
            {t("git.empty.hint")}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {repos.map((repo) => (
            <div key={repo.id} className="flex items-center justify-between rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-5 py-4 shadow-sm">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <p className="truncate font-mono text-sm font-semibold text-gray-900 dark:text-gray-100">{repo.repo_full_name}</p>
                </div>
              ...
              <div className="flex items-center gap-4">
                {repo.auto_deploy && (
                  <span className="inline-flex items-center rounded-full bg-green-50 dark:bg-green-950/40 px-2 py-0.5 text-xs font-medium text-green-700 dark:text-green-300 ring-1 ring-green-600/20">
