...
- [~] **S1 отдельный пул узлов под боксы: механизм проверен 2026-07-31, остался заказ пула.**
      Замер на проде (`docs/runbooks/box-isolation-probe.md`): taint
      `dada.io/box-pool=true:NoSchedule` + метка приняты узлом и держатся; под с toleration и
      `nodeSelector` — Running, тот же под **без** toleration — Pending, то есть гейт работает.
      Контроллера node-group внутри кластера **нет** (`beget-system` пуст, CRD нет), руками
      выставленные taint и метку никто не откатывает. Группы узлов у Beget существуют — метка
      `node-group.beget.com/name`, три штуки на четырёх узлах. Значит вопрос не «умеет ли
      кластер», а «закажем ли группу **только под боксы**».
      **Осталось человеку:** заказать такую группу у Beget. Не проверено намеренно: переживают
      ли taint и метка **пересоздание узла** группой — проверить сразу после заказа.
      До заказа боксы делят ядро с control plane платформы и гейт **не пройден**
