...
- [x] **B1 — РЕШЕНО (2026-07-30, по рекомендации): бокс = строка `environments` с
      `runtime='box'`.** Реализовано в `061_boxes.sql`; аудит ветвлений по `runtime` выполнен
      отдельной задачей с чеклистом (см. ФАЗА 2). Найдено 18 мест, исправлены два с ветвью по
      умолчанию, остальные безопасны по построению.
- [x] **B1. Бокс = строка `environments` (`runtime='box'`) или свой `ResourceKind`?**
