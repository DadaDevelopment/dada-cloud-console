...
- [ ] `063_box_usage.sql` — PK `(box_id, minute_start, kind)`, колонки минимум
      `box_id UUID, minute_start TIMESTAMPTZ, kind TEXT` (номер сдвинут: 060 занят
      `060_box_leads.sql`, 061–062 — фазой 2). **Простаивающая минута не создаёт строки вообще**
- [ ] **в этой же миграции:** перезапустить блок `DO` из `060_box_leads.sql`, создающий view
      `box_repeat_use_7d` (`CREATE OR REPLACE`, идемпотентен). До этого заголовочная метрика
      эксперимента отдаёт NaN и источника данных не имеет. Определение и SQL — в
      `docs/runbooks/box-funnel-metrics.md`; `TestCollectBoxRepeatUsePublishesTheMeasuredRatio`
      перестанет скипаться и начнёт пинить определение
