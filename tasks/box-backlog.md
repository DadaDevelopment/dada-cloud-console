...
- [ ] **отдельный пул узлов только под боксы, с taint и toleration** (зависит от S1).
      Механизм проверен на проде: taint держится, toleration пускает, под без toleration
      остаётся Pending. Осталось заказать группу узлов у Beget. Если пула нет — гейт не
      пройден, и подложку надо менять, а не «усиливать политиками»
