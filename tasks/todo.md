...
- [x] agent.json исключён из трейсов [dada-cloud `b10923dc`]: `excluded_urls` → `agent(-card)?\.json`, post-response flush тоже. Тег запинен [argo-infra `57f76ba9b`], 4 агента на `b10923dc` [live], digest пода = образ с патчем (проверен `docker run` по тому же digest). Перекат агентов под пробой: +1 ревизия RS у каждого, max 2 пода, 11/11 `200 completed`, 0 отказов [live]. Приёмка по Langfuse API не сделана: публичный API проекта забит 30/мин с этой машины другой сессией Claude (call-center, pid 51565, с 16.09) — `rate_limited` 7 попыток подряд.
...
- [x] Образ `ad9f93b6` уехал в прод 09-18, поверх него `4dd0a726` (#119) 09-19 12:07 MSK. Пробный ход с cost на корне — на НОВОМ проекте Langfuse: на старом бюджет-страж (day=25967>1500) глушит все трейсы, проверять нечем.
...
- [x] Jenkins #119 висел 1ч без агента: `0/4 nodes are available: 4 Insufficient memory` [live]. Под просит 3136Mi, лучший узел (86kcf) имел 3077Mi. Кто съел: `agent-sandbox-prod/tg-vibecoder-deploy` — 7 реплик при созданных 1 [argo-infra git log: 1→2→…→7, +1 на каждый деплой 09-08..09-16]. Причина [code `statusreconciler.go`]: имя-фолбэк атрибуции сложил `kagent/tg-vibecoder` (Deployment контроллера kagent) в снапшот приложения (`replicas: 8`, `namespaces: [agent-sandbox-prod, kagent]` [live БД]), а `doDeployImageVersion` рендерит replicas из снапшота. Фикс `1837c94a`: чужой namespace атрибутируется по имени только если у аппа нет ворклоада дома (тест RED без гарда, GREEN с). argo-infra `4fbef4b53`: replicas обратно 1. Рычага replicas в консоли НЕТ (только createApp/connectGitRepo) — отдельный гэп.
- [x] Доставлено 09-19: #119 `4dd0a726` (kagent-app image) + #120 `f94a28b8` зелёные; argo-infra `863e995b9` пин `kagent.agentImage.tag: 4dd0a726` (4/4 агента Running на нём [live]), `4d36997a6` бамп чарта `targetRevision: d3e79f58→f94a28b8` — чарт консоли пинится РУКАМИ в `apps/cloud-console/app.yaml`, без бампа ServiceMonitor/PrometheusRule из 4dd0a726 не рендерились вообще. agent-runtime `f94a28b8`.
- [x] Приёмка [live]: страж стартовал 09:00Z с `day=25967 month=81613 exceeded=true` → судья глух, `dada.telemetry=off` на каждом A2A; Prometheus видит `dada_langfuse_units{window=day}=25967`, `DadaLangfuseBudgetExceeded` pending→firing; `v2/metrics` observations по часам: 400-2000/ч всю ночь (все `@qa_target1_01..14`, 8k obs за 9ч — чей-то ночной QA-прогон) → 0 с 06:00Z. Roots от `qa_*` = 0 и score/ход = 2 на старом проекте не измерить (страж глушит всё) — мерить первым часом на новом проекте.
