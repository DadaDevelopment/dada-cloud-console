...
## Прод-прогон 2026-08-14

Фича была отгружена (консоль на образе `1521b0be`, миграции 118/119/120
применены) и при этом полностью бездействовала. Найдено и починено:

- **Каждый enqueue падал на 42P08.** Параметр, стоящий и в списке SELECT, и в
  предикате дедупликации, Postgres выводит дважды: `inconsistent types deduced
  for parameter $4`. Реконсилятор логировал по ошибке на базу в час
  (`db-tier: tick databases=21 retiered=0`), никто не ретраил и не алертил.
  Тот же дефект лежал в `enqueueDatabaseEnforcement` и `orderArchiveBucket` —
  просто ни разу не сработал, потому что тиров не было, а значит не было и
  переходов. Явные касты `$4::text`/`$5::jsonb`, покрыто
  `db_quota_enqueue_realdb_test.go` (реальный Postgres — иначе никак).
- **`orderArchiveBucket` вставлял операцию без `actor_id`** (NOT NULL). Любой
  запуск архива навсегда завис бы в фазе `sink`.
- **Запас под pg_repack мерился по раздутому отношению.** Замер:
  `VACUUM FULL` ужал таблицу 546 MB → 55 MB (каталог 568 MB → 77 MB), то есть
  rewrite копирует только живые строки. Старая оценка отказывала бы
  освобождать место ровно на тех томах, ради которых фича существует
  (`repackCopyBytes`).
- **Реконсилятор собирался посадить платформу на 5 ГБ read-only.** Все базы
  платформы (`cloud-console` 1.4 ГБ, `keycloak`, `nexus`, `powerdns`, `n8n`,
  `dadagent`, `user`) лежат в проектах орги `dada`, а она сама на плане
  `startup` → `tier=starter`. Константа `dbTierInternal` защищала только те
  базы, у которых тир УЖЕ `internal`, а на 14.08 все 21 сидели на дефолте XRD
  `unlimited` — то есть не защищала никого. Теперь орги из
  `DB_QUOTA_EXEMPT_ORGS` (дефолт `dada`, не пусто) получают `internal`
  активно — и на создании, и в реконсиляторе.

### Дыра в покрытии: odds-research

30 ГБ из 32 ГБ тома, растёт ~2 ГБ/сутки — и **невидима для продукта**:
существует только как сырой CR `databases.postgresql.sql.crossplane.io`, без
строки в `resource_snapshots` и без проекта в консоли. Ни реконсилятор, ни
watcher квоты её не увидят никогда. Отгруженная машинерия на этот кейс не
действует: нужен либо приём базы в консоль как `ServiceDatabaseV2` (после чего
через сутки grace она уедет в read-only — решение владельца, живой клиент),
либо операторский архив руками. Не решено.
...
### Второй прогон и баг замера (2026-08-14)

Прогон `1939993d-0c43-4cde-895a-52b66f921f3e` прошёл весь конвейер
`pending → sink → export → verify → delete → repack → done` за ~3 минуты,
231750 строк уехали в `s3://25f4da9f5cfe-bucket-phl69ycbl9zm/db-archive/...`,
таблица 295 МБ → 69 МБ, база 309 → 76 МБ. При этом строка прогона показывала
`bytes_freed = 0`.

Причина: фаза reclaim мерила размер до и после в одном вызове, но вызов,
создающий Job, возвращается раньше, чем Job отработает. До второго замера
доходит только последний повторный вход — и его «до» читается уже после
`VACUUM FULL`. То есть **все** завершённые прогоны сообщали владельцу, что
место не вернулось. Починено миграцией `122_db_archive_bytes_before.sql`:
размер до перезаписи штампуется в строку прогона перед созданием Job
(commit `993686a1`), переживает и повторный вход, и перезапуск пода консоли.

### Пруф на живом прогоне (2026-08-14, после деплоя `b84fa872`)

Прогон `daea034d-e5e0-4208-99b0-8c5caccce2ca` в песочнице, cutoff `2026-07-01`,
34500 строк:

```
bytes_before = 71966720
bytes_freed  = 36356096   (консоль показывает «34.7 MB»)
pg_total_relation_size('events') после = 35610624
```

71966720 - 35610624 = 36356096 — сходится байт в байт. Замер починен.

Песочница убрана: база `archive-probe` удалена через консоль (CR ушёл), но
`deletionPolicy: Orphan` оставил базу и роль `svc-archive-probe` на shard-0 —
снесены руками. Бакет `dada-archive` удалить нечем: в продукте **нет** DELETE
для S3-бакетов вообще (`router.go` знает только GET/POST и credentials), то есть
создание бакета — односторонняя дверь для любого пользователя. Заведено отдельной
задачей.
