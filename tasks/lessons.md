...
**Правило 23.** «Тесты зелёные» с грепом в конвейере — это не зелёные тесты.
`go test ./... | grep -v "^ok" | head -40` возвращает код выхода `head`, то
есть ноль всегда. Под этим фильтром спокойно проехал упавший
`TestOpenAPICoverage`: новый роут `/apps/{appName}/keep` был описан
аннотациями, но `swag init` после этого никто не гонял, и гейт покрытия спеки
честно падал. Правило: статус прогонa читается либо из `EXIT=$?` самой команды
`go test` (редирект в файл, не конвейер), либо из хвоста вывода целиком; грепу
можно верить только про содержимое, не про исход. И отдельно: любой новый
route в `router.go` — это `swag init -g cmd/server/main.go --parseInternal -o
internal/api/docs`, иначе CI красный.

**Правило 24.** В общем дереве с параллельными сессиями `git add <файл>` —
это ещё и коммит чужой недоделки. `handler.go`, `advisory_lock.go` и
`config.go` держали и мою строку, и половину чужой фичи, чей второй файл лежал
неотслеженным: закоммить я их целиком — main не собрался бы вообще, потому что
`StartAgentChatMemoryFolder` в коммите есть, а `agent_chat_memory.go` в нём
нет. Правило: перед коммитом смотреть `git diff` каждого файла на чужие ханки;
если они есть — сохранить рабочую копию, откатить файл на HEAD, наложить
только свои правки, закоммитить, вернуть копию. И проверять результат не в
рабочем дереве (там чужие файлы всё чинят), а в `git worktree add --detach
<tmp> HEAD` — там собирается ровно то, что уедет в CI.

**Правило 25.** `keep_firing_for` держит алерт сквозь провал условия, но не
сквозь смену идентичности серии. Продуктовые правила консоли считали сырой
per-target gauge (`dada_domain_hostname_pending_age_seconds`,
`dada_box_pool_available`), поэтому каждый алерт существовал по одному на
реплику и умирал вместе с именем пода. Для Alertmanager раскатка — это не
«тот же алерт продолжается», а «старые зарезолвились, новые загорелись»:
DadaCustomDomainStuck дал 24 серии за сутки на ОДИН застрявший хостнейм
(12 деплоев x 2 живые реплики), BoxPoolExhausted — 14. Правило: правило
алерта не должно нести `pod`/`instance` в наборе меток. Агрегируй
`max by (namespace, service, ...)` — привязка к сервису переживает раскатку.
И `sum` по репликам врёт в теле сообщения: DB-производный gauge одинаков на
всех репликах, `sum` показывал тройное число упавших операций.

**Правило 26.** Алерт про «ресурс кончился» обязан проверять, что ресурс
вообще заказывали. `BoxPoolExhausted` смотрел только на
`dada_box_pool_available == 0`, а `dada_box_pool_target` на проде равен нулю —
...
**Правило 28.** Алерт, который не называет объект, — это не алерт, а заявка на
запрос в базу. `DadaCustomDomainStuck` честно горел про застрявший домен почти
двое суток и писал «A custom hostname has been non-active for 1d 21h» — какой
именно, не знал никто, и разбор каждый раз начинался с ручного SQL. Причина
была не в шаблоне телеграма и не в правиле: сама метрика
`dada_domain_hostname_pending_age_seconds` была БЕЗ меток —
`promauto.NewGauge` поверх `min(created_at)` по всем pending-строкам. Агрегат
в SQL убивает идентичность раньше, чем до неё дотянется Prometheus, поэтому
шаблону оставались только `namespace` и `service`, приехавшие от скрейп-таргета.
Правило: метка, по которой человек пойдёт чинить (hostname, app, project, id
операции), должна попасть в НАБОР МЕТОК метрики; `by (...)` в правиле её только
протаскивает, создать не может. Проверка на этапе написания правила: прочитай
своё же `summary` вслух — если после него нужен `psql`, метрика недоделана.
Метка при этом обычно пользовательская, так что вместе с ней идут `Reset()`
каждый цикл (иначе починенный объект держит алерт горящим вечно) и `LIMIT`
на число серий (иначе массовый импорт кривых доменов — кардинальная бомба).
Фикс: `628e395`.

**Правило 32.** Новая страница консоли живёт в ДВУХ списках маршрутов:
`frontend/lib/agent-chat-links.ts` и `agentChatConsoleRoutes` в
`backend/internal/api/agent_chat.go`. Тест
`TestAgentChatConsoleRoutes_CoverEveryConsolePage` ходит по `frontend/app` и
падает, если backend отстал — билд #957 красный ровно из-за этого. Локальные
гейты фронта такое не ловят: гонять `go test ./internal/api` при добавлении
любой страницы, даже если Go-код «не трогали».

**Правило 33.** Новый под в чужом namespace сначала проверяй на
NetworkPolicy, а не на свой конфиг. В ns `databases` висит default-deny от
K10 (`podSelector: {}`, `policyTypes: [Ingress]`): у bitnami-шардов есть
собственные политики, а у только что задеплоенного pg-router не было — и он
отвечал на `127.0.0.1` внутри контейнера, но через Service давал ТАЙМАУТ, а
не `connection refused`. Таймаут при живом поде и Ready-пробе = дропнутые
пакеты, то есть политика; refused = процесс не слушает. Первым делом
`kubectl -n <ns> get netpol`, и только потом чтение своего конфига — иначе
полчаса уходит на подозрение к `%include`, который работал всё это время.
Побочно: `SHOW DATABASES` у PgBouncer не показывает строку `*` до первого
клиентского коннекта, записи из wildcard создаются лениво.

**Правило 34.** Ингресс без TLS ронял весь VM. `RenderNginxConf` печатала
канонический 443-vhost безусловно, а `ingressRebuildSpec` ставит
`TLS.Enabled = true` при первом же кастомном хосте — до того, как certbot
выпишет файлы. Пустой аргумент `ssl_certificate` для nginx — фатальная ошибка
конфига, поэтому контейнер уходил в CrashLoop и уносил маршрутизацию ВСЕХ
сайтов на машине, включая живые. Гейт должен стоять на наличии путей к файлам
(`CertPath != "" && KeyPath != ""`), а не на флаге `Enabled`: флаг выражает
намерение, файлы — факт. Заодно 80-й порт без сертификата обязан отдавать
`proxy_pass`, а не `return 301 https://` — редирект в несуществующий 443 это
цикл. Фикс: `dffb107f`.

**Правило 35.** Терминальный статус операции зависит от агента. Ops
portainer-agent заканчиваются `Ready` (`db.MarkReady`), gitops-agent —
`Committed`. Опрос «жду Committed» на DeleteAppServer висит вечно при уже
выполненном удалении. Список терминальных статусов — в
`frontend/components/app-servers/import-wizard.tsx:89`:
`Committed | Ready | Failed | Cancelled`. Сверяться с ним, а не с привычкой.

**Правило 36.** Preview на localhost может крутиться из ЧУЖОГО worktree.
`preview_list` показывает `cwd` сессии, а не рабочий каталог процесса —
`ps aux | grep next` показал, что :3100 запущен из
`.claude/worktrees/demo-fix`, поэтому правка в main-дереве «не применялась»
три перезагрузки подряд. Перед тем как искать баг в HMR или собственном коде:
`lsof -nP -iTCP -sTCP:LISTEN` + `ps aux`, чтобы узнать, чей это процесс.

**Правило 37.** Мусор не мигрируют — мусор удаляют. В батче переезда на
shard-0 уехала база `odds-research-pr-6` (284 МБ) — остаток мёртвого
PR-превью. Признак, который я обязан был проверить ДО переезда: имя
`*-pr-<N>`, пустой namespace, приложение удалено. Механизм, из-за которого
остаток пережил всё: превью воскресли как обычные окружения с
`is_ephemeral=false`, а и реаper (`ReapExpiredPreviewEnvs`), и
`DeletePreviewEnvironment` гейтятся ровно на этом флаге — значит окружение
становится невидимым для обоих путей уборки и живёт вечно. Второй зазор:
`doDeletePreviewEnv` сносит git-папки, снапшоты и строку окружения, но НЕ
сервисную базу — база остаётся сиротой на шарде. Перед любым батчем над
хранилищем сначала инвентаризация «что тут вообще должно жить», и только
потом переезд.

**Правило 38.** Переезд данных не переносит тех, кто ходит по старому
адресу. После переезда 32 баз на `shard-0` бэкапы четырёх клиентских БД
(`jira`, `recog`, `n8n`, `zerkalo`) снимались с БРОШЕННЫХ копий на shard-1 и
отчитывались `Ready`: ActionSet всегда целился в единственный
`DBBackupStatefulSet` из конфига. Restore тем же путём залил бы дамп в мёртвую
копию — клиент увидел бы «восстановили, а данных нет». Рядом жили ещё два
наследника старой карты: реестр `db_shards` держал `shard-0` closed +
`is_platform`, поэтому НОВЫЕ клиентские базы садились на инстанс, у которого
кончился диск, а `DB_ROUTER_DIRECT_SHARDS` по умолчанию содержал `shard-0` и
выдавал новому арендатору адрес шарда вместо роутера. Вывод: после любого
переезда перебирать ПОТРЕБИТЕЛЕЙ адреса — бэкап, restore, плейсмент, реестр,
дефолты конфига, метрики — и проверять их содержимым, а не цветом статуса.
Зелёный `Ready` значит «дамп снят», а не «снят с той базы, которой живёт
клиент».

## Переезд БД между шардами (08-10)

1. DSN или env пода, называющий инстанс напрямую, переезд НЕ замечает и
   продолжает писать в брошенную копию. Грепать argo-infra и env подов до
   cutover; старую копию переименовывать в `<db>--moved-to-shard-N`, а не
   удалять — забытый DSN обязан падать громко.
2. Секрет, отрендеренный чартом, kubectl'ом не чинится: править git И базу
   консоли, иначе следующий рендер вернёт старое.
3. Переезд везёт только роль владельца. `gateway_ro`, `dada`, `dada_stats` и
   их гранты пересоздавать руками.
4. База самого control plane через pgbouncer не ходит: advisory-локи
   сессионные.
5. Переезд собственной БД пишет `done` туда, где воркер этого уже не прочтёт.
6. Сверка «размер совпал» ничего не значит; `order by 1` в поколоночном хеше
   даёт ложный DIFFERS на неуникальной первой колонке — подтверждать
   `md5(string_agg(t::text,'' order by t::text))`.

## Git в общих worktree

7. Коммитить ТОЛЬКО по pathspec: `git add` + голый `git commit` замёл в пуш
   staged-удаление другой сессии. `git stash` в общем дереве запрещён.
8. Push отбит как non-fast-forward, хотя remote — предок? Соседняя сессия
   переписала локальную историю и уронила чужой коммит. Не чинить их ветку —
   пушить своим коммитом из отдельного detached worktree поверх
   `origin/<branch>`.

## Операции

9. У операции без heartbeat'а нет пути назад: смерть пода хоронила её в
   Processing навсегда. Перехват по возрасту обязателен, а безнадёжные
   (CR вне выводимого консолью пути) закрывать `Failed` — иначе перехват
   крутит их вечно.

## Переезд: потребители и имена

10. Фикс в коде — ещё не фикс в проде. Дефолт `DB_ROUTER_DIRECT_SHARDS`
    переписали в Go, а values продолжали передавать `shard-0` явно; живой под
    держал старое значение, а пункт считался закрытым. Проверять надо env
    работающего пода, а не дефолт в коде.

11. Переименование источника (`<db>--moved-to-shard-0`) НЕ гарантирует
    громкого падения забытого DSN: Crossplane, пока его MR смотрит на старый
    шард, видит «базы нет» и создаёт её заново — пустую. Забытый DSN логинится
    успешно и работает с базой без таблиц. Порядок обязателен:
    `providerConfigRef` на целевой шард → и только потом переименование.

12. Ищи потребителя по живым подам, а не по git. Один `kubectl get pods -A`
    с фильтром по env нашёл единственный прибитый DSN на брошенный инстанс
    там, где grep по репозиторию дал 15 файлов, из которых 13 — комментарии.

## Git как источник для деплоя

13. Путь ЧТЕНИЯ решает, что поедет в прод, — и он был единственным
    незащищённым. `doAdoptComposeStack` брал источник через `EnsureCloned()`,
    который выходит сразу при наличии `.git`: на прогретом агенте это no-op.
    Воркер прочитал скаффолд пятинедельной давности и откатил fin-data.pro на
    401. Путь ЗАПИСИ (`writeFilesCommitPush`) при этом годами делал
    `resetToRemoteHead`, из-за чего «гонки с git» считались закрытой темой.
    Читаешь git, чтобы решить, что деплоить, — зови `SyncHard()`.

14. Устаревшее чтение не даёт ошибки вообще. Файл в клоне ЕСТЬ, просто
    протухший: ни `not found`, ни лога, ни метрики. Единственный способ это
    поймать — тест, который двигает remote и проверяет, что читатель увидел
    новое содержимое (`internal/git/staleread_test.go`).

15. «Трансформация — идентичность» не равно «безопасно запускать». Я доказал
    round-trip compose на живом агрегате и что gitwatcher не воскресит фантом,
...
REST: одна запись переменной стоила четырёх вызовов и трёх UUID, а один из ответов
(`listApps`) физически не помещался в контекст. Это не косметика — каждый лишний шаг
это место, где можно перепутать id окружения и записать в прод вместо staging.

Rule: у инструмента, которым пользуется агент, адрес должен совпадать с тем, как
объект называют логи, git и люди. Имя резолвится ВНУТРИ сервера (тем же bearer,
теми же правилами видимости), ответы носят имена рядом с id и готовый `ref`,
списки по умолчанию тощие для агента и полные для UI. Неоднозначность (два
окружения, два проекта с одним именем) — отказ со списком кандидатов, никогда
не догадка: догадка и есть способ записать в прод.

Rule: разрушительная запись обязана иметь вопрос, который можно задать до неё.
`dry_run` рендерит, мержит, диффит и кладёт план в `validation_result`, не коммитя;
план описывает НАМЕРЕНИЕ (ключ, который собираются записать, подмешан в рендер),
но несёт только имена ключей — payload операции лежит открытым текстом.

## 2026-08-26 — Новый credential UI не считается единым, пока старые ключи невидимы

Ошибка: новый общий AI Gateway pool и его админский UI были выложены пустыми,
хотя рабочие platform credentials продолжали жить в legacy-таблице и конфиге.
Совместимость трафика сохранилась, но оператор не получил обещанную единую картину.

Rule: при замене secret-хранилища инвентаризация и идемпотентный импорт существующих
записей входят в acceptance criteria. До релиза UI должен показывать все активные
источники через безопасный masked hint, scope и provenance; отсутствие plaintext в
ответе проверяется отдельно от того, что рабочий ключ действительно виден.
