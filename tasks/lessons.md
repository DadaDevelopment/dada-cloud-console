...
**Правило 29.** Нельзя считать задачу про тревожный UI закрытой, просто скрыв
тревогу. После замечания владельца о том, что редкие клиентские сбои он сам не
увидит, стало ясно: URL-watcher's сигнал был одновременно плохим баннером и
единственным индикатором состояния. Убрать его из сводки без отдельной
наблюдаемой модели доступности означает сделать реальную поломку невидимой.
Правило: перед снижением заметности любого алерта составь цепочку
«детектор → хранилище → метрика/уведомление → экран → действие → recovery» и
проверь, что у владельца остаётся путь узнать о переходе состояния без
случайного открытия нужной страницы. Если нет — сперва строим правильный
канал состояния, затем убираем шум.

**Правило 30.** Для пользовательского времени нельзя выбирать UTC только по
сырому timestamp из API. Если интерфейс показывает время события человеку,
по умолчанию используй timezone браузера: в данном случае `19:01Z` корректно
отображается как `22:01` по Москве. Фиксированную зону добавляй только когда
продукт явно требует единой серверной шкалы и подписывай её в UI.

**Правило 31.** Не подменять наблюдаемый пользователем разрыв вторичной
гипотезой. В кейсе «CrashLoop прошёл, приложение уже Ready, красный баннер
остался» проверяем и называем прежде всего разъезд контуров состояния и
времени (`app_health_alerts.last_seen_at` против live Ready), а не
терминирующийся старый pod, если его не было в фактах. Фикс должен быть
привязан к успешному rollout: `DeployImageVersion` запускает наблюдение,
первый Ready без crash-state закрывает alert; защита от устаревших pod — только
дополнение и не объяснение инцидента.
